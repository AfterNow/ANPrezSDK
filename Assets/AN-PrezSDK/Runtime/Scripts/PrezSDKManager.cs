...
namespace AfterNow.PrezSDK
{
    ...
    public class PrezSDKManager : MonoBehaviour
    {
        ...
        public void StopSlide(bool now = false, Action action = null)
        {
            ...
            switch (slideTransition.animation)
            {
                ...
                case SlideAnimationType.ScaleOut:
                    ...
                    {
                        if (audioSource)
                            LeanTween.value(gameObject, 1, 0, slideTransition.animationDuration).setOnUpdate((val) =>
                            {
                                if (audioSource)
                    ...
                    foreach (var asset in PresentationManager.assets)
                    {
                        ...
                        var initialScale = asset.itemTransform.localScale.GetVector();
                        if (go != null)
                        {
        ...
        IEnumerator LoadSlide(int slideNo)
        {
            ...
            baseController.Callback_OnSlideStatusUpdate(SlideStatusUpdate.LOADING);
            ...
            baseController.Callback_OnSlideStatusUpdate(SlideStatusUpdate.LOADED);
