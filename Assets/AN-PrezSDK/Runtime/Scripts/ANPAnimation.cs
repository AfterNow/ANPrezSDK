    ...
    public class ANPAnimation
    {
        ...
        public void Play(bool gotoInitial, bool skipToEnd = false, bool stopAudio = true)
        {
            ...
            LeanTween.delayedCall(assetGO, _delay, () =>
            {
                ...
                if (gotoInitial)
                {
                    assetGO.SetInitialTransform(asset.itemTransform);
                }
        ...
        private void DoRegAnim(GameObject pAssetGO, bool skipToEnd, float _delay, float _animationDuration)
        {
            ...
            PresentationManager.initialPos = modelData.itemTransform.position.GetVector();
            PresentationManager.initialScale = modelData.itemTransform.localScale.GetVector();
            ...
            switch (model.animation)
            {
                case AnimationType.Appear:
                    ...
                    {
                        LeanTween.value(pAssetGO, 0, 1, totalLength - _delay).setOnComplete(Complete);
                    }
                    break;
