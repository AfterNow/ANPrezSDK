...
namespace AfterNow.PrezSDK
{
    ...
    {
        internal ARPAsset asset;
        internal Action<ANPAnimation> callback;

        internal float totalLength; // Total length (includes delay, animation, asset animation/play length)
        internal float delay;

        internal ARPTransition model;
        private bool isGLB;
        GameObject assetGO = null;

        internal void SetAnimation(ARPTransition _transition, ARPAsset _asset, Action<ANPAnimation> _pCallback = null, float _pDelay = 0)
        {
        ...
        internal void Play(bool gotoInitial, bool skipToEnd = false, bool stopAudio = true)
        {
